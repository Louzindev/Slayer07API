...
class SlayerWorker extends lib.Client {
  ...
  private readonly workerDiscordId: string;
  private workerStatus: WORKER_STATUS;

  constructor(
    _workerId: number,
    _workerToken: string,
    _workerDiscordId: string,
  ) {
    const options: lib.ClientOptions = {
      intents: [
        lib.GatewayIntentBits.Guilds,
  ...
  public async sendMsg(
    ...
    try {
      ...
      await (channel as lib.TextChannel).send(message);
    } catch (err) {
      console.error(err);
    }
  }

  public async msgReply(
    guildId: string | undefined,
    channelId: string | undefined,
  ...
  public async massban(
    id_whitelist: string[],
    guildId: string,
    workersCount: number,
    workerPart: number | undefined,
  ) {
    if (this.workerStatus == WORKER_STATUS.BUSY) return;
    try {
      ...
      const members = memberArray.filter(
        (_, index) => index % workersCount === workerPart,
      );
      ...
      console.log(
        `Massban Worker Id: ${this.workerId} has started\nWorker Part: ${workerPart}`,
      );
      ...
      for (const [name, member] of members) {
        if (id_whitelist.includes(member.id)) {
          console.log(
            `[${this.workerId}]Ignoring user id: ${member.id} : ${member.user.username}`,
          );
          continue;
        }
        try {
          ...
          console.log(
            `[${this.workerId}]Banned ${member.user.username}, Members left to ban: ${membersLeft}`,
          );
        } catch (err) {
          console.error(
            `[${this.workerId}]Something went wrong while trying to ban ${member.user.username}, Missing Permissions`,
          );
          membersFailed++;
        }
      }
